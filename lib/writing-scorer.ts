...
function scoreWordCount(
  ...
  if (taskType === "pte_swt") {
    ...
    if (words <= 75) return { score: 30, max, words, feedback: `Good length (${words} words). ✓` };
    return { score: 10, max, words, feedback: `Too long (${words} words) — SWT must be under 75 words.` };
  }
  ...
  if (words < min) return { score: 10, max, words, feedback: `Below minimum (${words}/${min} words) — write more.` };
  if (words < target) return { score: 20, max, words, feedback: `Meets minimum (${words} words). Aim for ${target}+ for stronger marks.` };
  return { score: 30, max, words, feedback: `Excellent word count (${words} words). ✓` };
}
...
function scoreVocabulary(words: string[]): { score: number; max: number; ttr: number; feedback: string } {
  ...
  if (ttr > 0.65) return { score: 25, max, ttr, feedback: "Excellent vocabulary variety. ✓" };
  if (ttr > 0.55) return { score: 20, max, ttr, feedback: "Good vocabulary variety." };
  if (ttr > 0.45) return { score: 15, max, ttr, feedback: "Moderate vocabulary — try using more synonyms." };
  if (ttr > 0.35) return { score: 10, max, ttr, feedback: "Low vocabulary variety — avoid repeating the same words." };
  return { score: 5, max, ttr, feedback: "Very repetitive vocabulary — use a wider range of words." };
}
...
function scoreCohesion(text: string): { score: number; max: number; count: number; words: string[]; feedback: string } {
  ...
  if (count >= 7) return { score: 25, max, count, words: found, feedback: "Excellent use of linking words. ✓" };
  if (count >= 5) return { score: 20, max, count, words: found, feedback: "Good cohesion — ideas are well-linked." };
  if (count >= 3) return { score: 15, max, count, words: found, feedback: "Some linking words used — add a few more." };
  if (count >= 1) return { score: 8, max, count, words: found, feedback: "Few linking words — use more (however, furthermore, in addition…)." };
...
function scoreSentenceVariety(text: string): { score: number; max: number; avgLength: number; feedback: string } {
  ...
  if (sentences.length < 2) return { score: 5, max, avgLength: 0, feedback: "Write more sentences to show variety." };
  ...
  if (avg < 5) return { score: 5, max, avgLength: avg, feedback: "Sentences are very short — write more complex sentences." };
  if (avg > 40) return { score: 8, max, avgLength: avg, feedback: "Sentences are very long — break some up for clarity." };

  if (stdDev >= 8 && avg >= 10 && avg <= 28) return { score: 20, max, avgLength: avg, feedback: "Excellent sentence variety. ✓" };
  if (stdDev >= 5) return { score: 15, max, avgLength: avg, feedback: "Good sentence variety." };
  if (stdDev >= 3) return { score: 10, max, avgLength: avg, feedback: "Some variety — mix shorter and longer sentences more." };
  return { score: 5, max, avgLength: avg, feedback: "Sentences are too uniform — vary your sentence structure." };
}
...
export function scoreTask(text: string, taskType: string): TaskScoreBreakdown {
  ...
  const total = wordCount.score + vocabulary.score + cohesion.score + sentenceVariety.score;
  return { wordCount, vocabulary, cohesion, sentenceVariety, total, band: totalToBand(total) };
}
