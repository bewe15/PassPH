...
export default async function MockExamPage({
  ...
  // ── Fetch latest attempt per section ──────────────────────────────────────
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{exam.exam} {exam.type} · {totalTime} min total</p>
            </div>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* ── Combined result card (shown when all sections done) ─────────── */}
        {allSectionsComplete && combinedBand !== null && (
          <div className="bg-gradient-to-br from-[#0a0e27] to-[#0f1535] rounded-2xl p-8 text-white">
            ...
            <div className="text-center mb-6">
              <p className={`text-6xl font-extrabold mb-2 ${getBandColor(combinedBand)}`}>{combinedBand}</p>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-700 mt-0.5">
                Free plan includes 1 mock exam. Upgrade to Pro for unlimited mock exams + AI Writing feedback.
              </p>
            </div>
        ...
        <Card>
          <CardContent className="py-5 px-6">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <p className="text-sm font-bold text-slate-900 mb-1">{exam.title}</p>
            ...
            <div className="flex flex-wrap gap-4 text-xs text-slate-500">
              ...
              <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-green-500" />{completedSections.length}/{exam.sections.length} completed</span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-3">
          ...
          {exam.sections.map((section, idx) => {
            ...
            return (
              <Card key={section.key} className={isDone ? "border-green-200 bg-green-50/30" : ""}>
                <CardContent className="py-0 px-0">
                  <div className="flex items-center gap-4 p-5">
                    {/* Step number / check */}
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${
                      isDone ? "bg-green-500 border-green-500 text-white" : "bg-slate-100 border-slate-200 text-slate-400"
                    }`}>
                      {isDone
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-xs text-slate-400">{section.durationMinutes} min · {section.questions} {section.key === "writing" ? "tasks" : "questions"}</span>
                      </div>
                      ...
                      {isDone && attempt && (
                        <div className="flex items-center gap-4 mt-2">
                          ...
                          <span className="text-xs text-slate-400">{attempt.score}/{attempt.total} · {attempt.time_taken}</span>
                      ...
                      {isDone ? (
                        <div className="flex flex-col gap-1.5">
                          <Link href={`/history/${attempt.id}`}>
                            <Button size="sm" variant="outline" className="text-xs w-full">View result</Button>
                          </Link>
        ...
        {/* ── Bottom CTA ───────────────────────────────────────────────────── */}
        {!allSectionsComplete && (
          <div className="bg-cyan-50 border border-cyan-200 rounded-xl px-5 py-4 flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-slate-900">
                {completedSections.length === 0
                  ? "Ready to start your mock exam?"
                  : `${exam.sections.length - completedSections.length} section${exam.sections.length - completedSections.length > 1 ? "s" : ""} remaining`}
              </p>
              <p className="text-xs text-slate-500 mt-0.5">
                Complete all sections to get your overall band score estimate.
              </p>
            </div>
            {completedSections.length === 0 && (
              <Link href={exam.sections[0].route} className="shrink-0">
        ...
        {allSectionsComplete && (
          <div className="text-center pt-2">
            <Link href="/dashboard">
              <Button variant="outline" className="gap-2">Back to dashboard</Button>
            </Link>
          </div>
        )}
      </main>
